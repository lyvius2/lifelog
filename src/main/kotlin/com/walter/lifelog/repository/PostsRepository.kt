...
interface PostsRepository : JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p WHERE p.categorySeq = :categorySeq AND p.createdAt < :createdAt ORDER BY p.createdAt DESC LIMIT 1")
    fun findPrevPost(categorySeq: Long, createdAt: LocalDateTime): Post?

    @Query("SELECT p FROM Post p WHERE p.categorySeq = :categorySeq AND p.createdAt > :createdAt ORDER BY p.createdAt ASC LIMIT 1")
